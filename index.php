<?php include_once (dirname(__FILE__).'/header_promint.php');?>
	
	        	<div class="col-md-12 col-xs-12 col-sm-12">
					<div class="blocoTituloPagina">
                		<div class="tituloPagina">Home</div>
            		</div>
            		<div style="clear:both; height:90px"></div>
    
            		<div class="col-md-4 col-md-offset-2 col-xs-12 col-sm-12">
            			<a class="" href="#"><img class="img-responsive" src="imagens/icoHome1.png" alt=""></a>
						<!--<div class="icoHome3"></div>-->
            		</div>
            		<div class="col-md-4 col-xs-12 col-sm-12">
						<a class="" href="#"><img class="img-responsive" src="imagens/icoHome2.png" alt=""></a>
            		</div>
            		<div class="col-md-2"></div>
            		<div class="col-md-2"></div>
	        	</div>
                <div class="col-md-12 col-xs-12 col-sm-12">
                    <div class="espacoFinal"></div>
                </div>
				
        	</div><!--row-->
        	
<?php include_once (dirname(__FILE__).'/footer.php');?>